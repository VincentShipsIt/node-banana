 ...
 */
function parseBase64DataUrl(base64DataUrl: string): { mimeType: string; data: Buffer } | null {
...
export function storeImage(base64DataUrl: string): string {
  ...
  if (!parsed) {
    throw new Error('Invalid base64 data URL format. Expected: data:{mimeType};base64,{data}');
  }
