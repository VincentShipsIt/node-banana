...
import type { ProviderInterface, ProviderModel } from './types';
...
export function getConfiguredProviders(): ProviderInterface[] {
  return Object.values(providerRegistry).filter((p): p is ProviderInterface => p?.isConfigured());
}
...
export function getAllProviders(): ProviderInterface[] {
  return Object.values(providerRegistry).filter((p): p is ProviderInterface => p !== undefined);
}
 ...
 */
export async function listAllModels(_apiKeys: ApiKeys = {}): Promise<ProviderModel[]> {
...
export async function searchAllModels(
  ...
  const providers = getAllProviders();
  const results = await Promise.allSettled(providers.map((p) => p.searchModels(query)));
