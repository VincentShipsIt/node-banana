...
import { parseJSONFromResponse, repairWorkflowJSON, validateWorkflowJSON } from '../validation';
...
describe('validation', () => {
    ...
    describe('nodes validation', () => {
      ...
      it('should accept all valid node types', () => {
        ...
        validTypes.forEach((type) => {
          ...
          expect(result.errors.filter((e) => e.path === 'nodes[0].type')).toHaveLength(0);
        });
      });
  ...
  describe('parseJSONFromResponse', () => {
    ...
    it('should throw error for invalid JSON', () => {
      expect(() => parseJSONFromResponse('not json')).toThrow('Could not parse JSON from response');
    });

    it('should throw error for empty string', () => {
      expect(() => parseJSONFromResponse('')).toThrow('Could not parse JSON from response');
    });
