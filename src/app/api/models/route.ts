...
import { getCachedModels, getCacheKey, setCachedModels } from '@/lib/providers/cache';
...
const GEMINI_IMAGE_MODELS: ProviderModel[] = [
  {
    ...
    name: 'Nano Banana',
    description:
      'Fast image generation with Gemini 2.5 Flash. Supports text-to-image and image-to-image with aspect ratio control.',
    provider: 'gemini',
  ...
  },
  {
    ...
    description:
      'High-quality image generation with Gemini 3 Pro. Supports text-to-image, image-to-image, resolution control (1K/2K/4K), and Google Search grounding.',
    provider: 'gemini',
    capabilities: ['text-to-image', 'image-to-image'],
    coverImage: undefined,
  },
...
export async function GET(request: NextRequest): Promise<NextResponse<ModelsResponse>> {
  ...
  const replicateKey =
    request.headers.get('X-Replicate-Key') || process.env.REPLICATE_API_KEY || null;
  ...
  for (const provider of providersToFetch) {
    ...
    const cacheKey =
      provider === 'replicate' ? getCacheKey(provider) : getCacheKey(provider, searchQuery);
