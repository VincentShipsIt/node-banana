...
function createReplicateResponse(
  models: Array<{ owner: string; name: string; description: string | null }>,
...
function createFalResponse(
  models: Array<{ id: string; name: string; category: string; description?: string }>,
  hasMore = false,
...
describe('/api/models route', () => {
  ...
  describe('basic functionality', () => {
    ...
    it('GET: should use API key from header over env var', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        createReplicateResponse([{ owner: 'stability-ai', name: 'sdxl', description: 'SDXL' }])
      );
  ...
  describe('caching behavior', () => {
    ...
    it('GET: should bypass cache when refresh=true', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        createFalResponse([{ id: 'fal-ai/flux-new', name: 'Flux New', category: 'text-to-image' }])
      );
  ...
  describe('capability inference', () => {
    it('GET: should infer text-to-video from video keywords', async () => {
      ...
      expect(response.status).toBe(200);
      const replicateModels = data.models.filter(
        (m: { provider: string }) => m.provider === 'replicate'
      );
      expect(
        replicateModels.every((m: { capabilities: string[] }) =>
    ...
    it('GET: should infer text-to-image as default for image models', async () => {
      ...
      const replicateModel = data.models.find(
        (m: { provider: string }) => m.provider === 'replicate'
      );
  ...
  describe('fal.ai category mapping', () => {
    it('GET: should map fal.ai categories to ModelCapability', async () => {
      ...
      expect(data.models.find((m: { id: string }) => m.id === 'fal-ai/flux')?.capabilities).toEqual(
        ['text-to-image']
      );
      expect(
        data.models.find((m: { id: string }) => m.id === 'fal-ai/img2img')?.capabilities
      ).toEqual(['image-to-image']);
      expect(data.models.find((m: { id: string }) => m.id === 'fal-ai/t2v')?.capabilities).toEqual([
        'text-to-video',
      ]);
      expect(data.models.find((m: { id: string }) => m.id === 'fal-ai/i2v')?.capabilities).toEqual([
        'image-to-video',
      ]);
    });
