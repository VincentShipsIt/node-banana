...
import type { ModelInput, ModelParameter } from '@/lib/providers/types';
import type { ProviderType } from '@/types';

// Cache for model schemas (10 minute TTL)
const schemaCache = new Map<
  string,
  { parameters: ModelParameter[]; inputs: ModelInput[]; timestamp: number }
>();
...
function isImageInput(name: string, prop: Record<string, unknown>): boolean {
  ...
  return name.endsWith('_image') || name.startsWith('image_') || name.includes('_image_');
}
 ...
 */
async function fetchReplicateSchema(modelId: string, apiKey: string): Promise<ExtractedSchema> {
 ...
 */
async function fetchFalSchema(modelId: string, apiKey: string | null): Promise<ExtractedSchema> {
  ...
  for (const pathObj of Object.values(spec.paths || {})) {
    ...
    if (jsonContent?.schema) {
      ...
      if (schema.$ref && typeof schema.$ref === 'string') {
        ...
        const resolvedSchema = spec.components?.schemas?.[refPath] as
          | Record<string, unknown>
          | undefined;
        if (resolvedSchema) {
        ...
        return NextResponse.json<SchemaErrorResponse>(
          {
            success: false,
            error:
              'Replicate API key required. Add REPLICATE_API_KEY to .env.local or configure in Settings.',
          },
          { status: 401 }
        );
      }
