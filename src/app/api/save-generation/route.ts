...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const _imageId = body.imageId; // Optional ID for carousel support
      ...
      try {
        ...
        if (contentLength) {
          ...
          if (size > MAX_CONTENT_SIZE) {
            throw new Error(
              `Content size ${size} bytes exceeds maximum allowed ${MAX_CONTENT_SIZE} bytes`
            );
          }
        }

        const contentType =
          response.headers.get('content-type') || (isVideo ? 'video/mp4' : 'image/png');
        ...
        if (arrayBuffer.byteLength > MAX_CONTENT_SIZE) {
          throw new Error(
            `Downloaded content size ${arrayBuffer.byteLength} bytes exceeds maximum allowed ${MAX_CONTENT_SIZE} bytes`
          );
        }
