...
async function generateWithGemini(
  ...
  console.log(
    `[API:${requestId}] Gemini generation - Model: ${model}, Images: ${images?.length || 0}, Prompt: ${prompt?.length || 0} chars`
  );
  ...
  for (const part of parts) {
    if (part.text) {
      console.error(
        `[API:${requestId}] Gemini returned text instead of image: ${part.text.substring(0, 100)}`
      );
      return NextResponse.json<GenerateResponse>(
        {
          success: false,
 ...
 */
function getParameterTypesFromSchema(
    ...
    for (const [genericName, patterns] of Object.entries(INPUT_PATTERNS)) {
      for (const pattern of patterns) {
        ...
        if (properties[pattern]) {
          ...
          const match = propertyNames.find(
            (name) =>
...
async function generateWithReplicate(
  ...
  console.log(
    `[API:${requestId}] Replicate generation - Model: ${input.model.id}, Images: ${input.images?.length || 0}, Prompt: ${input.prompt.length} chars`
  );
  ...
  console.log(
    `[API:${requestId}] Model version: ${version}, Dynamic inputs: ${hasDynamicInputs ? Object.keys(input.dynamicInputs!).join(', ') : 'none'}`
  );
  ...
  while (
    ...
    const pollResponse = await fetch(`${REPLICATE_API_BASE}/predictions/${currentPrediction.id}`, {
      headers: {
        Authorization: `Bearer ${apiKey}`,
      },
    });
 ...
 */
async function getFalInputMapping(
  modelId: string,
  apiKey: string | null
): Promise<FalInputMapping> {
  const paramMap: Record<string, string> = {};
  const arrayParams = new Set<string>();
  const schemaArrayParams = new Set<string>();
  ...
  try {
    ...
    for (const pathObj of Object.values(spec.paths || {})) {
      const postOp = (pathObj as Record<string, unknown>)?.post as
        | Record<string, unknown>
        | undefined;
...
async function generateWithFal(
  ...
  console.log(
    `[API:${requestId}] fal.ai generation - Model: ${input.model.id}, Images: ${input.images?.length || 0}, Prompt: ${input.prompt.length} chars`
  );

  const modelId = input.model.id;
  const hasDynamicInputs = input.dynamicInputs && Object.keys(input.dynamicInputs).length > 0;
  console.log(
    `[API:${requestId}] Dynamic inputs: ${hasDynamicInputs ? Object.keys(input.dynamicInputs!).join(', ') : 'none'}, API key: ${apiKey ? 'yes' : 'no'}`
  );

  // Fetch schema for type coercion and input mapping (only one API call)
  const { paramMap, arrayParams, schemaArrayParams, parameterTypes } = await getFalInputMapping(
    modelId,
    apiKey
  );
  ...
  console.log(
    `[API:${requestId}] Calling fal.ai API with inputs: ${Object.keys(requestBody).join(', ')}`
  );
  ...
  // Determine MIME type from response
  const contentType =
    mediaResponse.headers.get('content-type') || (isVideoModel ? 'video/mp4' : 'image/png');
 ...
 */
async function _generateWithFalQueue(
  requestId: string,
  apiKey: string | null,
  input: GenerationInput
): Promise<GenerationOutput> {
  console.log(
    `[API:${requestId}] fal.ai queue generation - Model: ${input.model.id}, Images: ${input.images?.length || 0}, Prompt: ${input.prompt.length} chars`
  );
  ...
  // Submit to queue
  console.log(
    `[API:${requestId}] Submitting to fal.ai queue with inputs: ${Object.keys(requestBody).join(', ')}`
  );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const hasPrompt = prompt || dynamicInputs?.prompt;
    const hasImages = images && images.length > 0;
    const hasImageInputs =
      dynamicInputs &&
      Object.keys(dynamicInputs).some((key) => key.includes('frame') || key.includes('image'));
    ...
    console.log(
      `[API:${requestId}] Provider: ${provider}, Model: ${selectedModel?.modelId || model}`
    );
    ...
    if (provider === 'replicate') {
      ...
      const replicateApiKey =
        request.headers.get('X-Replicate-API-Key') || process.env.REPLICATE_API_KEY;
      if (!replicateApiKey) {
        return NextResponse.json<GenerateResponse>(
          {
            success: false,
      ...
      let processedDynamicInputs: Record<string, string> | undefined;
    ...
    console.error(
      `[API:${requestId}] Generation error: ${errorMessage}${errorDetails ? ` (${errorDetails.substring(0, 200)})` : ''}`
    );
    return NextResponse.json<GenerateResponse>(
      {
        success: false,
