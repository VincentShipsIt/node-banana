...
export function ModelSearchDialog({
  ...
  const getCapabilityBadges = (capabilities: ModelCapability[]) => {
    ...
    capabilities.forEach((cap) => {
      ...
      if (label) {
        badges.push(
          <span key={cap} className={`text-[10px] px-1.5 py-0.5 rounded ${color}`}>
            {label}
          </span>
        );
      }
    ...
    >
      <div className="relative bg-neutral-800 border border-neutral-700 rounded-lg shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col mx-4">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-700">
          <h2 className="text-lg font-semibold text-neutral-100">Browse Models</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 gap-3">
              ...
              <p className="text-sm text-neutral-400 text-center max-w-xs">{error}</p>
              <button
                onClick={fetchModels}
            ...
            <div className="space-y-4">
              ...
              {filteredRecentModels.length > 0 && !searchQuery && (
                <div className="bg-neutral-700/30 rounded-lg p-3">
                  <h3 className="text-xs font-medium text-neutral-500 mb-2">Recently Used</h3>
