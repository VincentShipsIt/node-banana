...
import type { ModelInputDef, ProviderType } from '@/types';
  ...
  useEffect(() => {
    ...
    const fetchSchema = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/models/${encodedModelId}?provider=${provider}`, {
          headers,
        });
...
function ParameterInput({ param, value, onChange }: ParameterInputProps) {
  const displayName = param.name.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
  ...
  if (param.enum && param.enum.length > 0) {
    ...
    return (
      <div className="flex flex-col gap-0.5">
        <label className="text-[9px] text-neutral-400" title={param.description || undefined}>
          {displayName}
        </label>
        <select
  ...
  if (param.type === 'number' || param.type === 'integer') {
    ...
    return (
      <div className="flex flex-col gap-0.5">
        ...
        />
        {validationError && <span className="text-[8px] text-red-400">{validationError}</span>}
      </div>
    );
  }
