...
function ProviderBadge({ provider }: { provider: ProviderType }) {
  const providerName =
    provider === 'gemini' ? 'Gemini' : provider === 'replicate' ? 'Replicate' : 'fal.ai';
...
export function GenerateImageNode({ id, data, selected }: NodeProps<NanoBananaNodeType>) {
  ...
  const [_modelsFetchError, setModelsFetchError] = useState<string | null>(null);
  ...
  const _enabledProviders = useMemo(() => {
    ...
    if (
      providerSettings.providers.replicate?.enabled &&
  ...
  const hasExternalProviders = useMemo(() => {
    const hasReplicate =
      providerSettings.providers.replicate?.enabled && providerSettings.providers.replicate?.apiKey;
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/models?provider=${currentProvider}&capabilities=${capabilities}`,
        { headers }
      );
      if (response.ok) {
  ...
  const _handleProviderChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (provider === 'gemini') {
        ...
        const newSelectedModel: SelectedModel = {
          ...
          displayName:
            GEMINI_IMAGE_MODELS.find((m) => m.value === (nodeData.model || 'nano-banana-pro'))
  ...
  const _handleExternalModelChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const _handleModelChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      >
        ...
        {!isGeminiProvider && nodeData.inputSchema && nodeData.inputSchema.length > 0 ? (
          ...
          (() => {
            ...
            const renderedHandles = handles.map((handle, _index) => {
              ...
              {hasCarouselImages && (
                <div className="flex items-center justify-center gap-2 shrink-0">
                  ...
                  <span className="text-[10px] text-neutral-400 min-w-[32px] text-center">
                    {(nodeData.selectedHistoryIndex || 0) + 1} /{' '}
                    {(nodeData.imageHistory || []).length}
                  </span>
                  <button
