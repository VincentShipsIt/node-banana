...
import type { PredictedCostResult } from '@/utils/costCalculator';
...
describe('CostDialog', () => {
   ...
   */
  const createGeminiOnlyCost = (
    overrides: Partial<PredictedCostResult> = {}
  ): PredictedCostResult => ({
    totalCost: 0.463,
    breakdown: [
      {
  ...
  describe('Basic Rendering', () => {
    it('should render dialog with title', () => {
      render(
        <CostDialog predictedCost={createGeminiOnlyCost()} incurredCost={0} onClose={vi.fn()} />
      );
  ...
  describe('Gemini Cost Section', () => {
    ...
    it('should not show Gemini Cost section when no Gemini models', () => {
      render(
        <CostDialog predictedCost={createExternalOnlyCost()} incurredCost={0} onClose={vi.fn()} />
      );
  ...
  describe('External Providers Section', () => {
    it('should render External Providers section when external models exist', () => {
      render(
        <CostDialog predictedCost={createMultiProviderCost()} incurredCost={0} onClose={vi.fn()} />
      );
    ...
    it('should link to correct Replicate model page', () => {
      ...
      const links = screen.getAllByRole('link', { name: /View model/i });
      const replicateLink = links.find((link) =>
        link.getAttribute('href')?.includes('replicate.com')
      );
  ...
  describe('Incurred Cost Section', () => {
    it('should display formatted incurred cost', () => {
      render(
        <CostDialog predictedCost={createGeminiOnlyCost()} incurredCost={2.5} onClose={vi.fn()} />
      );
  ...
  describe('Reset Costs Button', () => {
    ...
    it('should show reset button when incurredCost is greater than 0', () => {
      render(
        <CostDialog predictedCost={createGeminiOnlyCost()} incurredCost={1.0} onClose={vi.fn()} />
      );
  ...
  describe('Close Behavior', () => {
    it('should call onClose when close button is clicked', () => {
      ...
      const { container } = render(
        <CostDialog predictedCost={createGeminiOnlyCost()} incurredCost={0} onClose={onClose} />
      );
