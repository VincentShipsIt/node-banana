...
import type { NanoBananaNodeData, ProviderSettings } from '@/types';
...
describe('GenerateImageNode', () => {
  ...
  describe('Fetch Models on Provider Change', () => {
    ...
    it('should not fetch models when provider is gemini', async () => {
      ...
      const fetchCalls = mockFetch.mock.calls.filter(
        (call) => typeof call[0] === 'string' && call[0].includes('/api/models?provider=gemini')
      );
      expect(fetchCalls.length).toBe(0);
    });
  });
