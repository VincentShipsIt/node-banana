...
import type { GenerateVideoNodeData, ProviderSettings } from '@/types';
...
describe('GenerateVideoNode', () => {
  ...
  const createNodeData = (
    overrides: Partial<GenerateVideoNodeData> = {}
        ...
        <TestWrapper>
          <GenerateVideoNode
            {...createNodeProps({
              outputVideo: 'data:video/mp4;base64,abc123',
              videoHistory: [
                { id: 'vid1', timestamp: Date.now(), prompt: 'test', model: 'kling-video/v1' },
              ],
              selectedVideoHistoryIndex: 0,
            })}
          />
  ...
  describe('Dynamic Input Handles', () => {
    ...
    describe('Multiple Image Inputs (common for video)', () => {
      it('should render multiple image handles for multi-frame video models', () => {
        ...
        const imageInputHandles = container.querySelectorAll(
          '[data-handletype="image"][class*="target"]'
        );
        expect(imageInputHandles.length).toBe(2);

        // Check schema names are set
        const schemaNames = Array.from(imageInputHandles).map((h) =>
          h.getAttribute('data-schema-name')
        );
  ...
  describe('Fetch Models on Mount', () => {
    it('should fetch models when provider is fal', async () => {
      ...
      await waitFor(() => {
        const fetchCalls = mockFetch.mock.calls.filter(
          (call) => typeof call[0] === 'string' && call[0].includes('/api/models?')
        );
    ...
    it('should request video capabilities when fetching models', async () => {
      ...
      await waitFor(() => {
        const fetchCalls = mockFetch.mock.calls.filter(
          (call) => typeof call[0] === 'string' && call[0].includes('text-to-video')
        );
        expect(fetchCalls.length).toBeGreaterThan(0);
      });
    });
